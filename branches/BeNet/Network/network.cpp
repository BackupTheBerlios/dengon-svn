...
CNetwork::~CNetwork()
{
	if(kill_thread(find_thread(NET_THREAD_RECV_RTCP)) != B_NO_ERROR)
		fprintf(stderr, "ERROR: Unable to kill NET_THREAD_RECV_RTCP: CNetwork::~CNetwork.\n");
...
status_t CNetwork::Initialize()
{
	...
	{	
		fprintf(stderr, "Error spawning NET_THREAD_RECV_RTCP.\n");
		return idRecvRTCP;
	}
...
status_t CNetwork::Quit()
{
	if(kill_thread(find_thread(NET_THREAD_RECV_RTCP)) != B_NO_ERROR)
	{
		fprintf(stderr, "ERROR: Unable to kill NET_THREAD_RECV_RTCP.\n");
		return B_ERROR;
	}
...
{
	fprintf(stderr, "ConnectFromUI to %s.\n", host);
	...
	if(find_thread(NET_THREAD_SEND_RTCP) == B_NAME_NOT_FOUND)
	{
		...
		{
			fprintf(stderr, "Error spawning NET_THREAD_SEND_RTCP.\n");
			return idSendRTCP;
		}
	...
	{
		fprintf(stderr, "ERROR: Unable to send NET_ID_CONNECT to NET_THREAD_SEND_RTCP: CNetwork::ConnectFromUI).\n");
		return B_ERROR;
	}
...
CNetwork::InitializeDiscussion()
{
	...
	if(find_thread(NET_THREAD_RECV_RTP) == B_NAME_NOT_FOUND)
	{
		...
		{
			fprintf(stderr, "Error spawning NET_THREAD_RECV_RTP.\n");
			return idRecvRTP;
		}		
	...
	if(find_thread(NET_THREAD_SEND_RTP) == B_NAME_NOT_FOUND)
	{
		...
		{
			fprintf(stderr, "Error spawning NET_THREAD_SEND_RTP.\n");
			return idSendRTP;
		}
	...
	if(find_thread(NET_THREAD_PIT) == B_NAME_NOT_FOUND)
	{
		...
		{
			fprintf(stderr,  "Error spawning NET_THREAD_PIT.\n");
			return idPit;
		}
		resume_thread(idPit);
	}
...
status_t CNetwork::Disconnect()
{
	...
	{
		fprintf(stderr, "ERROR: Unable to send NET_ID_DISCONNECT to NET_THREAD_SEND_RTCP: CNetwork::Disconnect).\n");
		return B_ERROR;;
	}
	...
	//	if (has_data(find_thread(NET_THREAD_SEND_RTP))) //OliverESP: improvement
		if (has_data(find_thread(NULL))) 
		{
			...
			case NET_ID_RAW:
				...
				{
					fprintf(stderr, "SENDRTP: ERROR: Unable to send RTP packet: CNetwork::SendModeRTP.\n" );
					return B_ERROR;	
				}
				break;
			case NET_ID_CODEC:
			...
			case NET_ID_MODEM:
				...
				break;
			default:
				fprintf(stderr, "SENDRTP: ERROR: Unknown message trying to be sent: CNetwork::SendModeRTP.\n");
				return B_ERROR;
				break;
			}
		}
...
status_t CNetwork::RecvModeRTP(void)
{
	...
	while(m_connectedRTP)
	{
		switch(protocol->RecvRTPPacket(payload,&size))
		{
			...
			default:
				fprintf(stderr, "RECVRTP: ERROR: Unknown message trying to be received: CNetwork::RecvModeRTP.\n");
...
status_t CNetwork::SendModeRTCP(void)
{
	...
	while(true)
	{
		...
		if (has_data(find_thread(NULL)))
		{
			...
			{
				case(NET_ID_CONNECT):
					fprintf(stderr,  "SENDRTCP: Sending CONNECT message.\n");
					protocol->SendRTCPPacket(NET_ID_CONNECT);
					break;
				case(NET_ID_DISCONNECT):
					fprintf(stderr,  "SENDRTCP: Sending DISCONNECT message.\n");
				...
				case(NET_ID_ALIVE):
#ifdef DEBUG_ALIVE
					fprintf(stderr,  "SENDRTCP: Sending ALIVE message.\n");
#endif	
					protocol->SendRTCPPacket(NET_ID_ALIVE);
					break;
				case(NET_ID_ACCEPT):
					fprintf(stderr,  "SENDRTCP: Sending ACCEPT message.\n");
					protocol->SendRTCPPacket(NET_ID_ACCEPT);
					break;
				case(NET_ID_REFUSE):
					fprintf(stderr,  "SENDRTCP: Sending REFUSE message.\n");
					protocol->SendRTCPPacket(NET_ID_REFUSE);
					break;
				case(NET_ID_BUSY):
					fprintf(stderr,  "SENDRTCP: Sending BUSY message.\n");
					protocol->SendRTCPPacket(NET_ID_BUSY);
					break;
				case(NET_ID_ABORT):
					fprintf(stderr,  "SENDRTCP: Sending ABORT message.\n");
					protocol->SendRTCPPacket(NET_ID_ABORT);
					break;
				default:
					fprintf(stderr,  "SENDRTCP: ERROR: Unknown message trying to be sent: CNetwork::SendModeRTCP.\n");
					return B_ERROR;
					break;
			}
		}
...
status_t CNetwork::RecvModeRTCP(void)
{
	...
	while(true)
	{
		...
		switch(protocol->RecvRTCPPacket(lRemoteAddress))
		{
			...
			case NET_ID_CONNECT:
			{
				fprintf(stderr, "RECVRTCP: Receiving Restez-vous chez vos parents.\n");
				...
				{					
					...
					{
						fprintf(stderr, "ERROR: Unable to send NET_ID_REFUSE to NET_THREAD_WAIT_CONNECTION: CNetwork::RecvModeRTCP).\n");
					}
					break;
				}
			...
			case NET_ID_DISCONNECT:
			{
				fprintf(stderr, "RECVRTCP: Receiving Disconnect.\n");
			...
			case NET_ID_ACCEPT:
			{
				fprintf(stderr, "Receive ACCEPT from remote.\n");
				if(send_data(find_thread(NET_THREAD_WAIT_CONNECTION), NET_ID_ACCEPT, NULL, 0) != B_NO_ERROR)
				{
					fprintf(stderr, "ERROR: Unable to send NET_ID_ACCEPT to NET_THREAD_WAIT_CONNECTION: CNetwork::RecvModeRTCP).\n");
				}
				break;
			}	
			case NET_ID_REFUSE:
			{
				fprintf(stderr, "Receive REFUSE from remote.\n");
			...
			case NET_ID_BUSY:
			{
				fprintf(stderr, "Receive BUSY from remote.\n");
				if(send_data(find_thread(NET_THREAD_WAIT_CONNECTION), NET_ID_BUSY, NULL, 0) != B_NO_ERROR)
				{
					fprintf(stderr, "ERROR: Unable to send NET_ID_BUSY to NET_THREAD_WAIT_CONNECTION: CNetwork::RecvModeRTCP).\n");
				}
				break;
			}
			case NET_ID_ABORT:
			{
				fprintf(stderr, "Receive ABORT from remote.\n");
			...
			{
#ifdef DEBUG_ALIVE
				fprintf(stderr,"RECVRTCP: Receiving Keep Alive.\n");
#endif
				lastTimeRecv = real_time_clock();
				break;
			}						
			default:
			{
				fprintf(stderr, "RECVRTCP: ERROR: Unknown message trying to be received: CNetwork::RecvModeRTCP.\n");
				break;
			}
		};
		...
		{
			fprintf(stderr, "RECVRTCP: Timeout.\n");
...
status_t CNetwork::Pit(void)
{
	...
	while(m_connectedRTP)
	{
		...
		{
			fprintf(stderr, "ERROR: Unable to send NET_ID_ALIVE to NET_THREAD_SEND_RTCP: CNetwork::Pit.\n");
			return B_ERROR;
		}
...
CNetwork::UpdateConnection(bool result)
{
	...
	if(result)	//  Ask user to accept the call.
	{
		...
		fprintf(stderr, "ERROR: Unable to send NET_ID_ACCEPT to NET_THREAD_SEND_RTCP: CNetwork::RecvModeRTCP).\n");
		...
		{
			fprintf(stderr,"InitializeDiscussion failed.\n");
			m_connectedRTCP = false;
		}
	}
	else
	{
		...
		if(send_data(find_thread(NET_THREAD_SEND_RTCP), NET_ID_REFUSE, NULL, 0) != B_NO_ERROR)
			fprintf(stderr, "ERROR: Unable to send NET_ID_REFUSE to NET_THREAD_SEND_RTCP: CNetwork::RecvModeRTCP).\n");
...
CNetwork::WaitConnection(void)
{
	...
	{
		fprintf(stderr, "ERROR: Unable to send NET_ID_ABORT to NET_THREAD_SEND_RTCP.\n");
		return B_ERROR;
	}
